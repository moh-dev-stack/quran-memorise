...
import { generateArabicOptions } from "@/lib/distractorGenerator";
import { isEmpty } from "@/lib/stringNormalizer";
import { getQuestionHash } from "@/lib/questionHash";

interface TransliterationToArabicModeProps {
  question: Question;
  allQuestions: Question[];
  onAnswer: (isCorrect: boolean) => void;
}

export default function TransliterationToArabicMode({
  question,
  allQuestions,
  onAnswer,
}: TransliterationToArabicModeProps) {
  ...
  const options = useMemo(() => {
    ...
    return generateArabicOptions(question.verse, allQuestions);
    ...
    <div className="w-full max-w-md space-y-4">
      <div className="w-full max-w-sm md:max-w-md rounded-xl shadow-lg bg-white p-6 mb-4">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-700 italic">{question.verse.transliteration}</p>
          <p className="text-base text-gray-600 mt-2">{question.verse.translation}</p>
        </div>
      </div>

      <div className="space-y-2">
        <h3 className="text-lg font-semibold text-gray-800 mb-3">
          Choose the correct Arabic verse:
        </h3>
        {options.map((option) => {
          ...
          return (
            ...
            >
              <div className="arabic-text text-xl break-words overflow-wrap-anywhere">{option.text}</div>
            </button>
          );
        })}
      </div>
