...
import { generateTransliterationOptions } from "@/lib/distractorGenerator";
import { isEmpty } from "@/lib/stringNormalizer";
import { getQuestionHash } from "@/lib/questionHash";

interface ArabicToTransliterationModeProps {
  question: Question;
  allQuestions: Question[];
  onAnswer: (isCorrect: boolean) => void;
}

export default function ArabicToTransliterationMode({
  question,
  allQuestions,
  onAnswer,
}: ArabicToTransliterationModeProps) {
  ...
  const options = useMemo(() => {
    ...
    return generateTransliterationOptions(question.verse, allQuestions);
    ...
    <div className="w-full max-w-md space-y-4">
      ...
      {isAnswered && (
        <Feedback
          ...
          correctAnswer={question.verse.transliteration}
